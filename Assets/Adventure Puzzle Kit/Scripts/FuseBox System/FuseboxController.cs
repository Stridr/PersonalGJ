...
namespace FuseboxSystem
{
    public class FuseboxController : MonoBehaviour
    {
        ...
        {
            if (FuseboxInventoryController.instance.inventoryFuses <= 0 && !powerOn)
            {
                AKAudioManager.instance.Play(fuseInsert);
            }

            if (FuseboxInventoryController.instance.inventoryFuses >= 1 && !powerOn)
            {
                if (!fuse1Inserted)
                {
                    fuseObject1.SetActive(true);
                    fuse1Inserted = true;
                    light1.GetComponent<Renderer>().material = greenButton;
                    FuseboxInventoryController.instance.MinusFuseUI();
                }
